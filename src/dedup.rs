...
pub fn group_by_hash<I>(paths: I) -> Vec<(Vec<u8>, Vec<DirEntry>)>
    where I: Iterator<Item = DirEntry>
{
    let groups = paths.filter_map(|entry| FileHash::from_entry(entry).map(|filehash| (filehash.hash, filehash.entry)).ok())
